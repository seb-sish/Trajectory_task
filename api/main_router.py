...
from schemas import (
    ScheduleSchema, IntervalSchema, TimeSlotSchema,
...
async def get_taken_slots_on_date(date_format: date) -> list[TimeSlotSchema]:
    try:
        ...
        day_slots = [slot for slot in schedule.timeslots if slot.day_id == day.id] if day else []
        return day_slots
    except (aiohttp.ClientError, ValidationError) as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def is_this_interval_free_on_date(date_format: date,
                                        interval: IntervalSchema) -> IsFreeIntervalSchema:
    try:
        schedule = await get_schedule()
        day = schedule.days.get(date_format)
        if not day:
...
async def find_free_interval(
    ...
    try:
        ...
        for day in schedule.days.values():
            day_slots = [slot for slot in schedule.timeslots
                          if slot.day_id == day.id] if day else []
            free_intervals = find_free_intervals(day, day_slots)
            for free_interval in free_intervals:
                if free_interval.duration() >= interval_duration:
                    end_time = (timedelta(hours=free_interval.start.hour,
        ...
        return FreeIntervalInScheduleSchema(founded=False,
                                             date=date(1900, 1, 1),
